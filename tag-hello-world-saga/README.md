...
This microservice handles the tagging of a Hello World aggregate.

During tagging it first verifies the Hello World aggregate exists. Then it verifies that the Hello World Tag aggregate exists. If the Hello World Tag aggregate does not exist, it creates it. Then it adds the Hello World Tag to the Hello World aggregate. Then it adds the Hello World identifier to the Hello World Tag aggregate. If either fails it rolls back the changes using compensating actions.
